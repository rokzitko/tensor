inline void channel1_only(MPO& H, const double Eshift, double epsishift1, double epsishift2, double epseff, const std::vector<double>& eps_,
                const std::vector<double>& v_, const params &p)
{
    ...
    // central matrix only has the second column, everything else is zero.
    {
        int i = ((length(H)+1)/2);
    ...
    for(auto i : range1( ((length(H)+1)/2)+1, length(H)-1 )){
        ...
        //W += p.sites.op("Id",i) * setElt(left(6), right(6));        
        //W += p.sites.op("Id",i) * setElt(left(7), right(7));        
        //W += p.sites.op("Id",i) * setElt(left(8), right(8));        
        //W += p.sites.op("Id",i) * setElt(left(9), right(9));        
    }
    ...
    //also identity
    {
        ...
        //W += p.sites.op("Id",  i) * setElt(left(1)); 
        W += p.sites.op("Id",  i) * setElt(left(2)); 
        //W += p.sites.op("Id",  i) * setElt(left(3)); 
        //W += p.sites.op("Id",  i) * setElt(left(4)); 
        //W += p.sites.op("Id",  i) * setElt(left(5)); 
        //W += p.sites.op("Id",  i) * setElt(left(6)); 
        //W += p.sites.op("Id",  i) * setElt(left(7)); 
        //W += p.sites.op("Id",  i) * setElt(left(8)); 
        //W += p.sites.op("Id",  i) * setElt(left(9)); 
    }
