inline void impurityTensor(MPO &H, const int i, const int imp_num, const std::vector<Index> links, const params &p){
    ...
    W += p.sites.op("Cup*F", i)     *setElt(left(3),right(2)) * (-1);
    W += p.sites.op("Cdn*F", i)     *setElt(left(4),right(2)) * (-1);
    W += p.sites.op("Cdagup*F", i)  *setElt(left(5),right(2)) * (+1);
    W += p.sites.op("Cdagdn*F", i)  *setElt(left(6),right(2)) * (+1);
...
inline void middleSC(MPO &H, const int first_site, const int SC_num, const std::vector<Index> links, const params &p){
    ...
    for (auto i : range1(first_site, first_site + p.SClevels - 1)){ // range1() IS INCLUSIVE, THUS -1!
        ...
        W += p.sites.op("Nupdn",i)          * setElt(left(1),right(2)) * (SC->g() * pow(SC->y(i), 2) + 2*SC->Ec());
        ...
        // hybridizations to the left
        W += p.sites.op("Cdagup*F",i)*setElt(left(7),right(2))* (+vl[j]);
        W += p.sites.op("Cdagdn*F",i)*setElt(left(8),right(2))* (+vl[j]);
        W += p.sites.op("Cup*F",   i)*setElt(left(9),right(2))* (-vl[j]);
        W += p.sites.op("Cdn*F",   i)*setElt(left(10),right(2))*(-vl[j]);
...
inline void Fill_SC_BathMPO_chain_alternating_SCFirst(MPO& H, const double Eshift, const params &p)
{
  ...
  links.push_back( Index() );
    ...
    //first site is a vector:
    {
        ...
        // local H on site
        W += p.sites.op("Ntot",i)  * setElt(right(2)) * (eps[i] + SC1->Ec()*(1-2*SC1->n0())); // here use index i
        W += p.sites.op("Nup",i)   * setElt(right(2)) * SC1->EZ()/2.0;
        W += p.sites.op("Ndn",i)   * setElt(right(2)) * (-1) * SC1->EZ()/2.0;
        W += p.sites.op("Nupdn",i) * setElt(right(2)) * (SC1->g() * pow(SC1->y(i), 2) + 2*SC1->Ec());
        W += p.sites.op("Id",i)    * setElt(right(2)) * Eshift;

        //hybridization to the right
        W += p.sites.op("Cdagup*F",i) * setElt(right(3))* (+vr[i]);
        W += p.sites.op("Cdagdn*F",i) * setElt(right(4))* (+vr[i]);
        W += p.sites.op("Cup*F",   i) * setElt(right(5))* (-vr[i]);
        W += p.sites.op("Cdn*F",   i) * setElt(right(6))* (-vr[i]);

        //SC pairing
        W += p.sites.op("Cdn*Cup",       i) * setElt(right(11)) * SC1->g() * SC1->y(i);
        W += p.sites.op("Cdagup*Cdagdn", i) * setElt(right(12)) * SC1->g() * SC1->y(i);
    ...
    for(auto i : range1(2, p.SClevels )){
        ...
        // local H on site
        W += p.sites.op("Ntot",i)           * setElt(left(1),right(2)) * (eps[i] + SC1->Ec()*(1-2*SC1->n0())); // here use index i
        W += p.sites.op("Nup",i)            * setElt(left(1),right(2)) * SC1->EZ()/2.0;
        W += p.sites.op("Ndn",i)            * setElt(left(1),right(2)) * (-1) * SC1->EZ()/2.0;
        W += p.sites.op("Nupdn",i)          * setElt(left(1),right(2)) * (SC1->g() * pow(SC1->y(i), 2) + 2*SC1->Ec());

        // hybridizations to the right
        W += p.sites.op("Cdagup*F",i)*setElt(left(1),right(3))* (+vr[i] );
        W += p.sites.op("Cdagdn*F",i)*setElt(left(1),right(4))* (+vr[i] );
        W += p.sites.op("Cup*F",   i)*setElt(left(1),right(5))* (-vr[i] );
        W += p.sites.op("Cdn*F",   i)*setElt(left(1),right(6))* (-vr[i] );

        //SC pairing
        W += p.sites.op("Cdn*Cup",i)        * setElt(left(1),right(11)) * SC1->g() * SC1->y(i);
        W += p.sites.op("Cdagup*Cdagdn",i)  * setElt(left(1),right(12)) * SC1->g() * SC1->y(i);
        ...
        W += p.sites.op("Cdagup*Cdagdn",i)*setElt(left(11),right(2)) * SC1->y(i);
        W += p.sites.op("Cdn*Cup",i)      *setElt(left(12),right(2)) * SC1->y(i);
    ...
    for (int i : range1( p.N - p.SClevels + 1, p.N-1 )){
        ...
        // local H on site
        W += p.sites.op("Ntot",i)           * setElt(left(1),right(2)) * (eps[i] + SCN->Ec()*(1-2*SCN->n0()));
        W += p.sites.op("Nup",i)            * setElt(left(1),right(2)) * SCN->EZ()/2.0;
        W += p.sites.op("Ndn",i)            * setElt(left(1),right(2)) * (-1) * SCN->EZ()/2.0;
        W += p.sites.op("Nupdn",i)          * setElt(left(1),right(2)) * (SCN->g() * pow(SCN->y(i), 2) + 2.0*SCN->Ec());

        // hybridizations to the left
        W += p.sites.op("Cdagup",i)*setElt(left(3),right(2))* vl[i];
        W += p.sites.op("Cdagdn",i)*setElt(left(4),right(2))* vl[i];
        W += p.sites.op("Cup",   i)*setElt(left(5),right(2))* vl[i];
        W += p.sites.op("Cdn",   i)*setElt(left(6),right(2))* vl[i];

        // SC pairing
        W += p.sites.op("Cdn*Cup",i)        * setElt(left(1),right(11)) * SCN->g() * SCN->y(i);
        W += p.sites.op("Cdagup*Cdagdn",i)  * setElt(left(1),right(12)) * SCN->g() * SCN->y(i);
        ...
        W += p.sites.op("Cdagup*Cdagdn",i) * setElt(left(11),right(2)) * SCN->y(i);
        W += p.sites.op("Cdn*Cup",i)       * setElt(left(12),right(2)) * SCN->y(i);
        W += p.sites.op("Ntot",i)          * setElt(left(13),right(2));
    }
    ...
    //site N is a vector again - same as before
    {
        ...
        W += p.sites.op("Ntot", i)  * setElt(left(1)) * (eps[i] + SCN->Ec()*(1-2*SCN->n0())); // use index i-1
        W += p.sites.op("Nup",  i)  * setElt(left(1)) * SCN->EZ()/2.0;
        W += p.sites.op("Ndn",  i)  * setElt(left(1)) * (-1) * SCN->EZ()/2.0;
        W += p.sites.op("Nupdn",i)  * setElt(left(1)) * (SCN->g() * pow(SCN->y(i), 2) + 2*SCN->Ec());

        W += p.sites.op("Id",    i) * setElt(left(2)) ;
        W += p.sites.op("Cdagup",i) * setElt(left(3)) * vl[i];
        W += p.sites.op("Cdagdn",i) * setElt(left(4)) * vl[i];
        W += p.sites.op("Cup",   i) * setElt(left(5)) * vl[i];
        W += p.sites.op("Cdn",   i) * setElt(left(6)) * vl[i];

        W += p.sites.op("Cdagup*Cdagdn",i) * setElt(left(11)) * SCN->y(i);
        W += p.sites.op("Cdn*Cup",      i) * setElt(left(12)) * SCN->y(i);
