inline void channel2_only(MPO& H, const double Eshift, double epsishift1, double epsishift2, double epseff, const std::vector<double>& eps_,
                const std::vector<double>& v_, const params &p)
{
    ...
    // sites 2 ... N-1 for the first channel. This is identity, but Fs are left for the 2ch - impurity coupling.
    for(auto i : range1(2,((length(H)+1)/2))){
