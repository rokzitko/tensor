inline void Fill_SCBath_MPO_Ec_V_SO(MPO& H, const double Eshift, const std::vector<double>& eps_,
...

{
    ...
    //first site is a vector:
    {
        ...
        W += p.sites.op("Ntot",i)  * setElt(right(2)) * epseff; // not eps_[i-1]!
    ...
    for(auto i : range1(2,length(H)-1)){
        ...
        W += p.sites.op("F",i) * setElt(left(11),right(11));
        W += p.sites.op("F",i) * setElt(left(12),right(12));
        W += p.sites.op("F",i) * setElt(left(13),right(13));
    }
